 ...
 * same-city + mutual-free-social-slot filtered), each with their open weekend
 * slots as day-labeled chips. Tapping a friend opens their page; tapping a
 * slot chip opens quick-plan prefilled with that date + slot.
 */
...
import { SLOT_LABEL, SLOT_START_HOUR, slotRangeLabel } from '@/lib/socialSlots';
          ...
          <View className="px-5 pt-2 gap-3">
            {freeFriends.map((f) => {
              ...
              return (
                <View
                  key={f.userId}
                  className="bg-card rounded-2xl border border-border/30 p-4 gap-3 shadow-sm"
                >
                  {/* Friend row → friend page */}
                  <Pressable
                    onPress={() => router.push(`/(app)/friend/${f.userId}`)}
                  ...
                  {/* Weekend slot chips → quick-plan */}
                  <View className="flex-row flex-wrap gap-1.5">
                    {f.weekendSlots.map((s) => (
                      <Pressable
                        key={`${s.date}-${s.slot}`}
                      ...
                      >
                        <Text className="font-sans text-xs font-semibold text-primary">
                          {format(parseLocal(s.date), 'EEE')} {SLOT_LABEL[s.slot].toLowerCase()}
                        </Text>
